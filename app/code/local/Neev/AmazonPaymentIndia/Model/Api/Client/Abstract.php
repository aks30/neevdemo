 ...
 */
abstract class Neev_AmazonPaymentIndia_Model_Api_Client_Abstract extends Neev_AmazonPaymentIndia_Model_Abstract {
    ...
    public function __construct() {
        ...
        $this->_config['UserAgent'] = 'Neev Checkout by Amazon Magento Extension/' . (string) Mage::getConfig()->getNode('modules/Neev_AmazonPaymentIndia/version')
    ...
    protected function _handleError($response, $status) {
        ...
        Mage::helper('amazonpaymentindia')->throwException($message, $status, array('area' => $this->_area, 'request_id' => $requestId));
    ...
    protected function _httpPost($params) {
        ...
        if ($socket = @fsockopen($scheme . $url['host'], $port, $errno, $errstr, 10)) {
            ...
            Mage::helper('amazonpaymentindia')->throwException(
                Mage::helper('amazonpaymentindia')->__('Unable to establish connection to host %s: %s', $url['host'], $errstr),
                $errno,
    ...
    protected function _pauseBeforeRetry($retries, $status) {
        if ($retries <= $this->_config['MaxErrorRetry']) {
            ...
            Mage::helper('amazonpaymentindia')->throwException(
                Mage::helper('amazonpaymentindia')->__('Maximum number of retry attempts to Amazon Payments API reached: %s', $retries),
                $status,
                array('area' => $this->_area)
            );
        }
    ...
    protected function _sign(array $params) {
        ...
        switch ($this->_config['SignatureMethod']) {
            ...
            case 'HmacSHA1':
                ...
                break;
            default:
                Mage::helper('amazonpaymentindia')->throwException('Non-supported signing method specified', null, array('area' => $this->_area));
        }
    ...
    public function __call($function, $params) {
        ...
        if (class_exists($classes['response'], true) && isset($params[0]) && $params[0] instanceof $classes['request'] && method_exists($classes['request'], 'convertToQueryString')) {
            ...
            Mage::helper('amazonpaymentindia')->throwException(Mage::helper('amazonpaymentindia')->__('Invalid method %s::%s (%s)', get_class($this), $function, print_r($params, 1)), null, array('area' => $this->_area));
        }
    }
    ...
    public function processRequest($request) {
        if (method_exists($request, 'convertToQueryString')) {
            ...
            Mage::helper('amazonpaymentindia')->throwException(Mage::helper('amazonpaymentindia')->__('Request %s cannot be converted to query string', get_class($this)), null, array('area' => $this->_area));
        }
    }
