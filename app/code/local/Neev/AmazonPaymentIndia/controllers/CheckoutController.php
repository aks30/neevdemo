 ...
 */
class Neev_AmazonPaymentIndia_CheckoutController extends Mage_Core_Controller_Front_Action {
    ...
    protected function _getPaymentMethod() {
        if (Mage::helper('amazonpaymentindia')->getConfigData('mode') == 'sandbox')
            return array('method' => 'amazonpaymentindia_sandbox');

        return array('method' => 'amazonpaymentindia');
    }
    ...
    protected function _expireAjax() {
        if (!$this->_getOnepage()->getQuote()->hasItems()
            ...
            || ($this->_getOnepage()->getCheckoutMehod()
                && !in_array($this->_getOnepage()->getCheckoutMehod(), array(Neev_AmazonPaymentIndia_Model_Abstract::CHECKOUT_METHOD_AMAZON, Mage_Checkout_Model_Type_Onepage::METHOD_CUSTOMER)))) {
    ...
    protected function _getAmazonCheckoutApi() {
        if (null === $this->_amazonCheckoutApi) {
            $this->_amazonCheckoutApi = Mage::getModel('amazonpaymentindia/api_checkout');
        }
        return $this->_amazonCheckoutApi;
    }

    protected function _getAmazonPrice($amount) {
        return Mage::getModel('amazonpaymentindia/api_model_checkout_price', array(
            'Amount' => Mage::helper('amazonpaymentindia')->sanitizePrice($amount)
        ));
    }
    ...
    protected function _getReviewHtml() {
        $layout = $this->getLayout();
        $update = $layout->getUpdate();
        $update->load('amazonpaymentindia_checkout_review');
        $layout->generateXml();
        $layout->generateBlocks();
        $output = $layout->getOutput();
        return $output;
    ...
    protected function _getOnepage() {
        return Mage::getSingleton('amazonpaymentindia/checkout');
    }
    ...
    protected function _validateCountry($countryCode) {
        ...
        if (Mage::getStoreConfigFlag(Neev_AmazonPaymentIndia_Model_Abstract::XML_PATH_COUNTRY_ALLOW)) {
            $availableCountries = explode(',', strtoupper(Mage::getStoreConfig(Neev_AmazonPaymentIndia_Model_Abstract::XML_PATH_SPECIFIC_COUNTRY)));
        } else {
    ...
    protected function _logException(Exception $exception) {
        Neev_AmazonPaymentIndia_Model_Logger::logException(
    ...
    public function indexAction() {
        ...
        $this->_getOnepage()->saveCheckoutMethod(Neev_AmazonPaymentIndia_Model_Abstract::CHECKOUT_METHOD_AMAZON);
        ...
        try {
            ...
            if (isset($addressList[0])) {
                ...
                if (count($customerName) > 1) {
                    ...
                    $firstname = Mage::helper('amazonpaymentindia')->__('n/a');
                    $lastname = reset($customerName);
                }

                $regionId = Mage::helper('amazonpaymentindia')->getRegionId($addressModel->getStateOrProvinceCode(), $addressModel->getCountryCode());
                ...
                if (Mage::getSingleton('eav/config')->getAttribute('customer', 'prefix')->getIsRequired()) {
                    $prefix = Mage::helper('amazonpaymentindia')->getConfigData('default_customer_prefix');
                }
                ...
                $address = array(
                    ...
                    'lastname' => $lastname,
                    'company' => '',
                    'email' => Mage::helper('amazonpaymentindia')->getConfigData('email_placeholder'),
                    'street' => $street,
                ...
                if ($this->_validateCountry($addressModel->getCountryCode())) {
                    ...
                    $result['error'] = 1;
                    $result['message'] = Mage::helper('amazonpaymentindia')->__('Sorry. We do not deliver to this country. Please select another shipping address.');
                }
    ...
    public function saveOrderAction() {
        ...
        $result = array();
        try {
            Mage::helper('amazonpaymentindia')->lockExceptionLogging();
            ...
            if (!$order->getId()) Mage::helper('amazonpaymentindia')->throwException('Cannot load order with given ID', 0, array('area' => 'Inline Checkout'));

            $purchaseContractId = $this->getRequest()->getParam('purchaseContractId', null);
            $itemList = Mage::getModel('amazonpaymentindia/api_model_checkout_itemList');
            ...
            foreach ($order->getItemsCollection() as $item) {
                ...
                $amazonItem = Mage::getModel('amazonpaymentindia/api_model_checkout_purchaseItem', array(
                    'MerchantItemId' => $item->getId(),
                    'SKU' => Mage::helper('amazonpaymentindia')->sanitizeSku($item->getSku()),
                    'Title' => $item->getName(),
                    'UnitPrice' => array('Amount' => Mage::helper('amazonpaymentindia')->sanitizePrice($itemUnitPrice)),
            ...
            $amazonCharges = Mage::getModel('amazonpaymentindia/api_model_checkout_charges');
            if (abs($order->getBaseDiscountAmount()) > 0) {
                $amazonPromotion = Mage::getModel('amazonpaymentindia/api_model_checkout_promotion', array(
                    'PromotionId' => (trim($order->getCouponCode()) ? trim($order->getCouponCode()) : Mage::helper('amazonpaymentindia')->__('Discount')),
                    'Description' => (trim($order->getDiscountDescription()) ? trim($order->getDiscountDescription()) : Mage::helper('amazonpaymentindia')->__('Discount')),
                    'Discount' => array('Amount' => abs($order->getBaseDiscountAmount()))
                ));
                $promotionList = Mage::getModel('amazonpaymentindia/api_model_checkout_promotionList');
            ...
            if (!$setItemsResult) Mage::helper('amazonpaymentindia')->throwException('Error when performing SetItems call', 0, array('area' => 'Inline Checkout'));

            $setContractChargesResult = $this->_getAmazonCheckoutApi()->setContractCharges($purchaseContractId, $amazonCharges);
            if (!$setContractChargesResult) Mage::helper('amazonpaymentindia')->throwException('Error when performing SetContractCharges call', 0, array('area' => 'Inline Checkout'));
            ...
            if (!is_null($amazonOrderIdList)) {
                ...
                Mage::helper('amazonpaymentindia')->throwException('Error when performing CompletePurchaseContract call', 0, array('area' => 'Inline Checkout'));
            }
            ...
            Mage::helper('amazonpaymentindia')->unlockExceptionLogging();
