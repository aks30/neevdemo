 ...
 */
class Neev_AmazonPaymentIndia_Model_Api_Server_Iopn extends Neev_AmazonPaymentIndia_Model_Api_Server_Abstract {
    ...
    protected function _validateParams(array $params = array()) {
        if (!isset($params['UUID']) || !isset($params['Timestamp']) || !isset($params['AWSAccessKeyId']) || !isset($params['Signature'])) {
            Mage::helper('amazonpaymentindia')->throwException('Either UUID or Timestamp or AWSAccessKeyId or Signature missing in the request', 500, array('area' => $this->_area));
        }

        if (!$this->_verifyAccessKey($params['AWSAccessKeyId'])) {
            Mage::helper('amazonpaymentindia')->throwException('Invalid AWS Access Key ID', 500, array('area' => $this->_area));
        }

        if (abs(strtotime($params['Timestamp']) - $this->_getTimestamp()) > 900) {
            Mage::helper('amazonpaymentindia')->throwException('Rejecting notification due to timestamp range overflow', 403, array('area' => $this->_area));
        }

        if (!$this->_verifySignature($params['UUID'], $params['Timestamp'], $params['Signature'])) {
            Mage::helper('amazonpaymentindia')->throwException('Rejecting notification due to invalid signature', 403, array('area' => $this->_area));
        }

        if (!$this->_validateXml($this->_extractXml($params))) {
            Mage::helper('amazonpaymentindia')->throwException('Validation of XML against schema failed', 500, array('area' => $this->_area));
        }
