 ...
 */
class Neev_AmazonPaymentIndia_Model_Observer extends Neev_AmazonPaymentIndia_Model_Abstract {

    protected static $_lockApiCalls = false;
    protected static $_lockFeedSending = false;

    protected function _getAmazonManager() {
        return Mage::getSingleton('amazonpaymentindia/manager');
    }

    protected function _assertSchedule($schedule, $timeDiff) {
        switch ($schedule) {
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_NEVER:
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_15_MINUTES:
                if ($timeDiff >= 900) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_30_MINUTES:
                if ($timeDiff >= 1800) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_1_HOUR:
                if ($timeDiff >= 3600) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_2_HOURS:
                if ($timeDiff >= 7200) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_4_HOURS:
                if ($timeDiff >= 14400) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_8_HOURS:
                if ($timeDiff >= 28800) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_12_HOURS:
                if ($timeDiff >= 43200) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_1_DAY:
                if ($timeDiff >= 86400) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_2_DAYS:
                if ($timeDiff >= 172800) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_3_DAYS:
                if ($timeDiff >= 259200) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_7_DAYS:
                if ($timeDiff >= 604800) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_14_DAYS:
                if ($timeDiff >= 1209600) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_15_DAYS:
                if ($timeDiff >= 1296000) return true;
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::SCHEDULE_30_DAYS:
    ...
    public function cleanAmazonLog() {
        Neev_AmazonPaymentIndia_Model_Logger::cleanLogs();
    }
    ...
    public function cancelOrderInAmazon($observer) {
        try {
            ...
            if (!($e instanceof Neev_AmazonPaymentIndia_Exception)) {
        ...
        try {
            ...
            if ($this->_isAmazonPaymentMethod($order->getPayment()->getMethod()) &&
                ...
                if ($order->getShipmentsCollection()->count()) {
                    ...
                    $order->addStatusHistoryComment(Mage::helper('amazonpaymentindia')->__('<strong>%s</strong>: Order shipment has been notified to Amazon', 'Amazon MWS'));
    ...
    public function logApiCall($observer) {
        ...
        Neev_AmazonPaymentIndia_Model_Logger::logApiCall($host, $action, $requestMethod, $headers, $get, $post, $file, $responseCode, $response);
        return $this;
    }
