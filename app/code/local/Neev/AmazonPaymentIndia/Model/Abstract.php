...
abstract class Neev_AmazonPaymentIndia_Model_Abstract {

    // general settings
    const XML_PATH_ACTIVE                   = 'amazonpaymentindia/general/active';
    const XML_PATH_IOPN_ACTIVE              = 'amazonpaymentindia/general/iopn_active';
    const XML_PATH_MARKETPLACE              = 'amazonpaymentindia/general/marketplace';
    const XML_PATH_EMAIL_PLACEHOLDER        = 'amazonpaymentindia/general/email_placeholder';
    const XML_PATH_DEFAULT_PREFIX           = 'amazonpaymentindia/general/default_prefix';
    const XML_PATH_WEIGHT_UNIT              = 'amazonpaymentindia/general/weight_unit';   // TB added
    const XML_PATH_CUT_SKU                  = 'amazonpaymentindia/general/cut_sku';
    const XML_PATH_SANDBOX_MODE             = 'amazonpaymentindia/general/sandbox_mode';
    const XML_PATH_NEW_ORDER_STATUS         = 'amazonpaymentindia/general/new_order_status';
    const XML_PATH_ORDER_CONFIRMATION       = 'amazonpaymentindia/general/order_confirmation';

    // authorization data
    const XML_PATH_MERCHANT_ID              = 'amazonpaymentindia/general/merchant_id';
    const XML_PATH_ACCESS_KEY_ID            = 'amazonpaymentindia/general/access_key';
    const XML_PATH_SECRET_KEY               = 'amazonpaymentindia/general/secret_key';

    // availability settings
    const XML_PATH_COUNTRY_ALLOW            = 'amazonpaymentindia/general/sallowspecific';
    const XML_PATH_SPECIFIC_COUNTRY         = 'amazonpaymentindia/general/specificcountry';

    // Amazon MWS settings
    const XML_PATH_MWS_ORDER_UPDATE         = 'amazonpaymentindia/api/order_update';
    const XML_PATH_REPORT_SCHEDULE          = 'amazonpaymentindia/api/report_schedule';
    const XML_PATH_MWS_USE_TOKEN            = 'amazonpaymentindia/api/use_token';
    const XML_PATH_FEED_BATCHING            = 'amazonpaymentindia/api/feed_batching';
    const XML_PATH_FEED_SCHEDULE            = 'amazonpaymentindia/api/feed_schedule';
    const XML_PATH_LAST_FEED_SENDING        = 'amazonpaymentindia/api/last_feed_sending';
    const XML_PATH_LAST_REPORT_PROCESSING   = 'amazonpaymentindia/api/last_report_processing';
    const XML_PATH_LAST_ORDER_PROCESSING    = 'amazonpaymentindia/api/last_order_processing';

    // debugging options
    const XML_PATH_ALLOWED_IPS              = 'amazonpaymentindia/general/allow_ips';
    const XML_PATH_LOGGER_ENABLED           = 'amazonpaymentindia/logger/active';
    const XML_PATH_LOGGER_CLEAN_AFTER       = 'amazonpaymentindia/logger/clean_after_day';

    const GLOBAL_CONFIG_CBA_NODE            = 'global/creativestyle/amazonpaymentindia';
    ...
    protected $_modulePrefix = 'amazonpaymentindia';
    ...
    private static function _getApiEndpointUrl($api, $marketplace = 'en_US', $mode = 'live') {
        switch ($api) {
            ...
            default:
                Mage::helper('amazonpaymentindia')->throwException('API name invalid or not specified');
        }
    }
    ...
    protected function _generateHmacSignature($data, $algorithm = self::HMAC_SHA1_ALGORITHM) {
        switch (strtolower($algorithm)) {
            ...
            case self::HMAC_SHA256_ALGORITHM:
                break;
            default :
                Mage::helper('amazonpaymentindia')->throwException('Non-supported signing method specified');
        }
    ...
    protected function _getAmazonPaymentMethods() {
        return array('amazonpayments', 'amazonpaymentindia', 'amazonpaymentindia_sandbox');
    }
    ...
    public static function getConfigData($key, $params = array()) {
        switch ($key) {
            ...
            case 'iopn_schema':
                return Mage::getBaseDir() . DS . str_replace('<codePool>', (string)Mage::getConfig()->getNode('modules/Neev_AmazonPaymentIndia/codePool'), (string)Mage::getConfig()->getNode(self::GLOBAL_CONFIG_CBA_NODE . '/api/iopn/schema_path'));

            // returns IOPN url, if in sandbox non-secure URL can be used
            case 'iopn_url':
                switch (self::getConfigData('mode')) {
                    case 'sandbox':
                        return Mage::getUrl('amazonpaymentindia/process/iopn', array('_nosid' => true));
                    default:
                        return Mage::getUrl('amazonpaymentindia/process/iopn', array('_nosid' => true, '_forced_secure' => true));
                }
            ...
            case 'marketplace_id':
                switch (self::getConfigData('marketplace')) {
                    ...
                    default:
                        Mage::helper('amazonpaymentindia')->throwException('Unknown marketplace');
                }
