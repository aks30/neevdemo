 ...
 */
class Neev_AmazonPaymentIndia_Model_Manager extends Neev_AmazonPaymentIndia_Model_Abstract {
        ...
        switch ($type) {
            case 'orders':
                if (!isset($this->_ordersApi[$storeId])) {
                    $this->_ordersApi[$storeId] = Mage::getModel('amazonpaymentindia/api_marketplace_orders');
                }
                return $this->_ordersApi[$storeId];

            case 'reports':
                if (!isset($this->_reportsApi[$storeId])) {
                    $this->_reportsApi[$storeId] = Mage::getModel('amazonpaymentindia/api_marketplace_reports');
                }
                return $this->_reportsApi[$storeId];

            case 'feeds':
                if (!isset($this->_feedsApi[$storeId])) {
                    $this->_feedsApi[$storeId] = Mage::getModel('amazonpaymentindia/api_marketplace_feeds');
                }
    ...
    protected function _submitBatchFeeds($feedType, $directory, $merchantId = null) {
        ...
        $allowedMessageType = '';
        switch ($feedType) {
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_ACKNOWLEDGEMENT_DATA:
                $allowedMessageType = 'OrderAcknowledgement';
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_FULFILLMENT_DATA:
                $allowedMessageType = 'OrderFulfillment';
                break;
            case Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_PAYMENT_ADJUSTMENT_DATA:
        ...
        if ($messageId > 0) {
            ...
            // log feed
            Neev_AmazonPaymentIndia_Model_Logger::logFeed(
                $feedType,
                $result,
                    ...
                    if (!empty($order) && $order->getId()) {
                        ...
                        if (isset($message->OrderReport->BillingData->BuyerName)) {
                            ...
                            if (count($buyerName) > 1) {
                                ...
                                $buyerFirstname = Mage::helper('amazonpaymentindia')->__('n/a');
                        ...
                        if (isset($message->OrderReport->BillingData->Address)) {
                            ...
                            if (isset($message->OrderReport->BillingData->Address->CountryCode)) {
                                if (isset($message->OrderReport->BillingData->Address->StateOrRegion)) {
                                    $order->getBillingAddress()->setRegionId(Mage::helper('amazonpaymentindia')->getRegionId((string)$message->OrderReport->BillingData->Address->StateOrRegion, (string)$message->OrderReport->BillingData->Address->CountryCode));
                        ...
                        if ($order->getState() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                            if ($order->getStatus() == 'pending_amazon') {
                                $order->addStatusToHistory(Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage::helper('amazonpaymentindia')->__('<strong>%s</strong>: Order details have been updated', 'Amazon MWS'), false);
                            }
                            $status = self::getConfigData('new_order_status');
                            $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, $status, Mage::helper('amazonpaymentindia')->__('<strong>%s</strong>: Order amount has been authorized by Amazon', 'Amazon MWS'), false);
        ...
        if ($reportList->issetReportInfo()) {
            ...
            foreach ($reports as $reportListItem) {
                if ($reportListItem->issetReportType() && $reportListItem->issetReportId() &&
                    ($reportListItem->getReportType() == Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Reports_Abstract::REPORT_TYPE_ORDERS_DATA)) {
                    ...
                    if ($processedOrders = $this->_processAmazonReportsApiCallResult($report)) {
                        ...
                        Neev_AmazonPaymentIndia_Model_Logger::logReport(
    ...
    public function updateCanceledOrders() {
        $storeId = Mage::app()->getStore()->getId();
        if (version_compare(Mage::helper('amazonpaymentindia')->getMagentoVersion(), '1.4.1') >= 0) {
        ...
        if ($amazonOrder->issetAmazonOrderID()) {
            ...
            if (!empty($order) && $order->getId()) {
                ...
                if ($order->getStatus() == 'pending_amazon') {
                    ...
                    $order->setState($state, $state, Mage::helper('amazonpaymentindia')->__('<strong>%s</strong>: Order details have been updated', 'IOPN'), false);
                }
                $order->save();
                if (!$order->getEmailSent() && self::getConfigData('order_confirmation')) $order->sendNewOrderEmail();
            }
        } else {
            Mage::helper('amazonpaymentindia')->throwException('Amazon Order ID not provided');
        }
    }
    ...
    public function cancelOrder($amazonOrder, $sender = 'IOPN') {
        if ($amazonOrder->issetAmazonOrderID() || $amazonOrder->issetAmazonOrderId()) {
            ...
            if (!empty($order) && $order->getId()) {
                $invoices = $order->getInvoiceCollection();
                $order->getPayment()->setMessage(Mage::helper('amazonpaymentindia')->__('<strong>%s</strong>: Authorization has been voided by Amazon', $sender));
                if ($invoices->count()) {
                    foreach ($invoices as $invoice) {
                        $invoice->void();
                    }
        ...
        if ($amazonOrder->issetAmazonOrderID()) {
            ...
            if (!empty($order) && $order->getId()) {
                if ($order->getState() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                    ...
                    if ($amazonOrder->issetBillingAddress()) {
                        ...
                        if ($amazonOrder->getBillingAddress()->issetCountryCode()) {
                            if ($amazonOrder->getBillingAddress()->issetState()) {
                                $order->getBillingAddress()->setRegionId(Mage::helper('amazonpaymentindia')->getRegionId($amazonOrder->getBillingAddress()->getState(), $amazonOrder->getBillingAddress()->getCountryCode()));
                    ...
                    $order->getPayment()->getMethodInstance()->setStore($order->getStoreId())->authorize($order->getPayment(), $order->getBaseTotalDue());
                    $status = self::getConfigData('new_order_status');
                    $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, $status, Mage::helper('amazonpaymentindia')->__('<strong>%s</strong>: Order amount has been authorized by Amazon', 'IOPN'), false);
                    $invoices = $order->getInvoiceCollection();
                    if ($invoices->count()) {
                        foreach ($invoices as $invoice) {
                            $transactionSave->addObject($invoice);
    ...
    public function sendCancellationNotify($order) {
        ...
        if ($this->_isAmazonPaymentMethod($order->getPayment()->getMethod()) && $order->getPayment()->getLastTransId()) {
            ...
            if (!self::getConfigData('feed_batching')) {
                $result = $this->_submitFeed(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_ACKNOWLEDGEMENT_DATA, $filePath);
                Neev_AmazonPaymentIndia_Model_Logger::logFeed(
                    Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_ACKNOWLEDGEMENT_DATA,
                    $result,
                    trim($envelope->asXML()),
    ...
    public function sendShipmentNotify($order) {
        ...
        if (($order->getState() == Mage_Sales_Model_Order::STATE_COMPLETE) && $this->_isAmazonPaymentMethod($order->getPayment()->getMethod()) && $order->getPayment()->getLastTransId()) {
            ...
            if ($shipment) {
                ...
                if (!self::getConfigData('feed_batching')) {
                    $result = $this->_submitFeed(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_FULFILLMENT_DATA, $filePath);
                    Neev_AmazonPaymentIndia_Model_Logger::logFeed(
                        Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_FULFILLMENT_DATA,
                        $result,
                        trim($envelope->asXML()),
                        $order->getStore()->getId()
                    );
                ...
                Mage::helper('amazonpaymentindia')->throwException('No shipment assigned to order #' . $order->getIncrementId());
            }
        }
        return $result;
    }
    ...
    public function sendRefundNotify($payment) {
        ...
        if ($this->_isAmazonPaymentMethod($order->getPayment()->getMethod()) && $order->getPayment()->getLastTransId()) {
            ...
            foreach ($creditmemo->getAllItems() as $item) {
                ...
                $envelope->Message[0]->OrderAdjustment->AdjustedItem[$i]->AdjustmentReason = Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::ADJUSTMENT_REASON_CUSTOMER_RETURN;
                $envelope->Message[0]->OrderAdjustment->AdjustedItem[$i]->ItemPriceAdjustments->Component[$j]->Type = 'Principal';
                $principalAmount = $envelope->Message[0]->OrderAdjustment->AdjustedItem[$i]->ItemPriceAdjustments->Component[$j]->addChild('Amount', Mage::helper('amazonpaymentindia')->sanitizePrice($item->getBaseRowTotalInclTax() - $item->getBaseDiscountAmount()));
                ...
                if (!$i) {
                    if ($creditmemo->getBaseShippingAmount() > 0) {
                        ...
                        $shippingAmount = $envelope->Message[0]->OrderAdjustment->AdjustedItem[$i]->ItemPriceAdjustments->Component[$j]->addChild('Amount', Mage::helper('amazonpaymentindia')->sanitizePrice($creditmemo->getBaseShippingAmount()));
            ...
            if (!self::getConfigData('feed_batching')) {
                $result = $this->_submitFeed(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_PAYMENT_ADJUSTMENT_DATA, $filePath);
                Neev_AmazonPaymentIndia_Model_Logger::logFeed(
                    Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_PAYMENT_ADJUSTMENT_DATA,
                    $result,
                    trim($envelope->asXML()),
                    $order->getStore()->getId()
                );
    ...
    public function sendFeeds() {
        ...
        // acknowledge feeds
        $this->_submitBatchFeeds(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_ACKNOWLEDGEMENT_DATA, $this->_queueDir['acknowledge'], $merchantId);

        // cancellation feeds
        $this->_submitBatchFeeds(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_ACKNOWLEDGEMENT_DATA, $this->_queueDir['cancel'], $merchantId);

        // shipment feeds
        $this->_submitBatchFeeds(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_ORDER_FULFILLMENT_DATA, $this->_queueDir['shipment'], $merchantId);

        // refund feeds
        $this->_submitBatchFeeds(Neev_AmazonPaymentIndia_Model_Api_Model_Marketplace_Feeds_Abstract::FEED_TYPE_POST_PAYMENT_ADJUSTMENT_DATA, $this->_queueDir['refund'], $merchantId);
