 ...
 */
abstract class Neev_AmazonPaymentIndia_Model_Api_Model_Abstract extends Neev_AmazonPaymentIndia_Model_Abstract {
    ...
    private function _isApiModelClass($type) {
        return preg_match('/^Neev_AmazonPaymentIndia_Model_Api_Model_/', $type);
    }

    private function _isApiObject($value) {
        if (gettype($value) == 'object') {
            return preg_match('/^Neev_AmazonPaymentIndia_Model_Api_Model_/', get_class($value));
        }
        return false;
    }
        ...
        if (!is_null($data)) {
            if ($this->_isHashTable($data)) {
                ...
                Mage::helper('amazonpaymentindia')->throwException(
                    Mage::helper('amazonpaymentindia')->__('Unable to construct Amazon API model: %s from provided data. Please be sure to pass associative array or DOMElement.', get_class($this)),
                    null,
